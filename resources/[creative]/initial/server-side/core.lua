...
-----------------------------------------------------------------------------------------------------------------------------------------
Hypex = {}
Tunnel.bindInterface("initial",Hypex)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKINIT
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.CheckInit()
	local source = source
	...
	if Passport then
        ...
        if not Account.initial then
			...
			if vehicle[1] then
                ...
                TriggerClientEvent("Notify",source,"verde",NotifyText,10000)
			end
        end
        else
            if NotifyConfig then
            TriggerClientEvent("Notify",source,"vermelho",NotifyFailed,10000)
            end	
	    end
    end
end