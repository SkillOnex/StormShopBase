...
-----------------------------------------------------------------------------------------------------------------------------------------
Hypex = {}
Tunnel.bindInterface("player",Hypex)
...
-- PUTVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.putVehicle(Network)
	if NetworkDoesNetworkIdExist(Network) then
		local Vehicle = NetToEnt(Network)
		if DoesEntityExist(Vehicle) then
			local vehSeats = 10
	...
	while true do
		...
		if Distance < 6.1 and not IsPedInAnyVehicle(ped) then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(closestVehicle) + GetEntityForwardVector(closestVehicle),
				...
				GetDistanceBetweenCoords(GetEntityCoords(closestVehicle) +
					GetEntityForwardVector(closestVehicle) * -1, GetEntityCoords(ped), true) then
...
CreateThread(function()
	while true do
		...
		if Vehicle.Vehicle ~= nil then
			local ped = PlayerPedId()
			if IsControlPressed(0, 83) and GetEntityHealth(ped) > 100 and IsVehicleSeatFree(Vehicle.Vehicle, -1) and
				...
				while true do
					...
					if not IsDisabledControlPressed(0, 83) then
						DetachEntity(ped, false, false)
...
function takeHostage()
	...
	if not canTakeHostage then
		print("You need a pistol with ammo to take a hostage at gunpoint!")
		TriggerEvent("Notify", "amarelo", "Voce precisa de uma arma com munição.", "Atenção", 5000)
	end
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		if closestPlayer ~= -1 then
			...
			holdingHostage = true
			TriggerServerEvent('refem:sync', closestPlayer, lib, lib2, anim1, anim2, distans, distans2, height,
				target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget, attachFlag)
		else
			TriggerEvent("Notify", "amarelo", "Nenhum jogador pra ser feito de refém.", "Atenção", 5000)
		end
	end
	canTakeHostage = false
end

RegisterCommand("refem", takeHostage)

RegisterNetEvent('refem:syncTarget')
AddEventHandler('refem:syncTarget',
...
RegisterNetEvent('refem:syncMe')
AddEventHandler('refem:syncMe', function(animationLib, animation, length, controlFlag, animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('refem:cl_stop')
AddEventHandler('refem:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false
	...
	while true do
		...
		if holdingHostage or beingHeldHostage then
			...
			while not IsEntityPlayingAnim(GetPlayerPed(-1), takeHostageAnimDictPlaying, takeHostageAnimNamePlaying, 3) do
				TaskPlayAnim(GetPlayerPed(-1), takeHostageAnimDictPlaying, takeHostageAnimNamePlaying, 8.0, -8.0, 100000,
...
function GetClosestPlayer(radius)
	...
	if closestDistance <= radius then
		...
		TriggerEvent("Notify", "amarelo", "Nenhum jogador proximo!!.", "Atenção", 5000)
		return nil
	end
end
	...
	while true do
		...
		if holdingHostage then
			...
			if IsEntityDead(GetPlayerPed(-1)) then
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("refem:stop", target)
				Wait(100)
				releaseHostage()
			end
			...
			DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z, "Aperte [G] para soltar, [H] para matar")
...
local function cavalinho(source, args)
	if not piggyback.InProgress then
		local closestPlayer = GetClosestPlayer(3)
		if closestPlayer then
			local targetSrc = GetPlayerServerId(closestPlayer)
			if GetEntityHealth(GetPlayerPed(closestPlayer)) >= 101 and not Player(closestPlayer).state.handCuffed then
				local success = vSERVER.RequestPiggback(targetSrc)
			else
				success = true
			end
			if not success then
			...
			if targetSrc ~= -1 then
				...
				TriggerEvent("Notify", "negado", "Não existe ninguem proximo para carregar.")
			end
		else
			TriggerEvent("Notify", "negado", "Não existe ninguem proximo para carregar.")
		end
...
RegisterCommand("cavalinho", cavalinho)
...
AddEventHandler("Piggyback:syncTarget", function(targetSrc)
	...
	AttachEntityToEntity(PlayerPedId(), targetPed, 0, piggyback.personBeingPiggybacked.attachX,
		piggyback.personBeingPiggybacked.attachY, piggyback.personBeingPiggybacked.attachZ, 0.5, 0.5, 180, false,
	...
	while true do
		if piggyback.InProgress then
			if piggyback.type == "beingPiggybacked" then
				if not
					IsEntityPlayingAnim(PlayerPedId(), piggyback.personBeingPiggybacked.animDict,
						piggyback.personBeingPiggybacked.anim, 3) then
					TaskPlayAnim(PlayerPedId(), piggyback.personBeingPiggybacked.animDict,
						piggyback.personBeingPiggybacked.anim, 8.0, -8.0, 100000,
						piggyback.personBeingPiggybacked.flag, 0, false, false, false)
				end
			elseif piggyback.type == "piggybacking" then
				if not
					IsEntityPlayingAnim(PlayerPedId(), piggyback.personPiggybacking.animDict,
						piggyback.personPiggybacking.anim, 3) then
					TaskPlayAnim(PlayerPedId(), piggyback.personPiggybacking.animDict,
						piggyback.personPiggybacking.anim, 8.0, -8.0, 100000, piggyback.personPiggybacking.flag, 0,
						false, false, false)
				end
			end
		end
