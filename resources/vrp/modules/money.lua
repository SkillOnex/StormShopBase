...
function vRP.GetFine(source)
    ...
    local Consult = vRP.Query("fines/Check1", {Passport = source})
...
-- GIVEFINE
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.GiveFine(Passport,data,hora,Amount,text)
    local Amount = parseInt(Amount)
    local Source = vRP.Source(Passport)
    if Amount > 0 then
        local nome = "Marcelo Souza"
        vRP.Query("fines/Add",{ NULL, Passport = Passport, Name = nome , Date = data, Hour = hora , Value = Amount , Message = text })        
        if Characters[Source] then
