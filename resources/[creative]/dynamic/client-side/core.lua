...
RegisterCommand("tencodeFunctions",function()
	...
	if (LocalPlayer["state"]["Policia"]) or LocalPlayer["state"]["Pc"] and not IsPauseMenuActive() then
		...
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and LocalPlayer["state"]["Route"] < 900000 then
			...
			if LocalPlayer["state"]["Policia"] or LocalPlayer["state"]["Pc"] then
...
RegisterCommand("emergencyFunctions",function()
	if (LocalPlayer["state"]["Policia"] or LocalPlayer["state"]["Paramedico"]) or LocalPlayer["state"]["Civil"] and not IsPauseMenuActive() then
		if not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not menuOpen and LocalPlayer["state"]["Route"] < 900000 then

			local Ped = PlayerPedId()
			if GetEntityHealth(Ped) > 100 then
				if not IsPedInAnyVehicle(Ped) then
					exports["dynamic"]:AddButton("Anunciar","Fazer um anúncio para todos.","dynamic:EmergencyAnnounce","",false,true)
