...
-- ENTRANCEHOMES
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.entranceHomes(TheftName,v,Interior,Theft)
	DoScreenFadeOut(0)
	...
	if Theft then
		...
		HomesTheft["Policia"] = GetGameTimer() + 15000
	...
	while true do
		...
		if HomesTheft["Theft"] ~= "" and HomesTheft["Open"] ~= "" then
			...
			if not IsPedInAnyVehicle(ped) then
				...
				if speed > 2 and GetGameTimer() >= HomesTheft["Policia"] and not HomesTheft["Called"] then
					HomesTheft["Policia"] = GetGameTimer() + 15000
