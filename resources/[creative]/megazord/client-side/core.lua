...
CreateThread(function()
	while true do
		if LocalPlayer["state"]["Active"] and not LocalPlayer["state"]["Admin"] then
			...
			if not Injected["DamageModify"] and not LocalPlayer["state"]["Megazord"] and (GetPlayerMeleeWeaponDamageModifier(Pid) > 1.0 or GetPlayerVehicleDamageModifier(Pid) > 1.0 or GetPlayerWeaponDamageModifier(Pid) > 1.0 or GetPlayerMeleeWeaponDefenseModifier(Pid) > 1.0 or GetWeaponDamageModifier(CurrentWeapon) > 1.0) then
			...
			if not Injected["Invincible"] and not LocalPlayer["state"]["Invincible"] and (GetPlayerInvincible_2(Pid) or GetPlayerInvincible(Pid)) then
...
AddEventHandler("esx:getSharedObject",function()
	if not Injected["ESX"] then
		vSERVER.Warning("ESX Shared Objects")
		Injected["ESX"] = true
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
