	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				contents = `<item>${name} - ${Passport}</item>${description !== "false" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r><br>Tipo Sangüineo: <r>${idBlood}</r><br>Rolepass: <r>${idRolepass}</r><br>Diamantes: <r>${idGemstone}</r><br>Validade: <r>${idVality}</r><br>Premium: <r>${idPremium}</r></legenda>`;
			}
