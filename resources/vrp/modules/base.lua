...
-- function vRP.BannedFull(data)
--     local Consult = vRP.Query("bannedsFull/GetBannedFull",{ license = data.steamid, fivem = data.license, fivem2 = data.license2, discord = data.discord, xbl = data.xbl, liveid= data.liveid, ip = data.ip })
--     if Consult and Consult[1] then
--         if Consult[1].time <= os.time() then
--             vRP.Query("banneds/RemoveBanned",{ license = data.steamid })
--             return false
--         end
--         return true
--     end
...
vRP.getUserId = vRP.Passport -- adaptação para aceitar a função.
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERS
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("Queue:Connecting",function(source,identifiers,deferrals)
    ...
    if Identity then
        ...
        -- old elseif not vRP.Banned(Identity) and not vRP.BannedFull(checks) then 
