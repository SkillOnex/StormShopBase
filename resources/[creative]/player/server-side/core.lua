...
AddEventHandler("player:Demand",function(OtherSource)
	...
	if Passport and OtherPassport then
		local Keyboard = vKEYBOARD.keySingle(source,"Valor da Cobrança")
		if Keyboard and vRP.Passport(OtherSource) then
			local Price = parseInt(Keyboard[1])
			if vRP.Request(OtherSource,"Aceitar a cobrança de <b>$"..parseFormat(Price).."</b>","Sim","Não") then
				if vRP.PaymentFull(OtherPassport,Price,true) then
					vRP.GiveBank(Passport,Price,true)
			end
			else
				TriggerClientEvent("Notify",source,"vermelho","Pedido recusado.",5000)
			end
		end
	end
end)
...
-----------------------------------------------------------------------------------------------------------------------------------------
local playerCarry = {}
function Hypex.CarryPlayer()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		if vRP.HasService(Passport,"Paramedico") or vRP.HasService(Passport,"Policia")  or vRP.HasService(Passport,"Admin") then
...
AddEventHandler("player:Informations",function()
	...
	local Passport = vRP.Passport(source)
	if Passport then
		if not vRP.HasGroup(Passport,"Policia") and not vRP.HasGroup(Passport,"Paramedico") then
			if vRP.Request(source,"Quer receber informações por <b>$10.000</b>?","Sim, por favor","Não, decido depois") then
				if vRP.ConsultItem(Passport,"dollarsroll",10000) and vRP.TakeItem(Passport,"dollarsroll",10000) then
					local Paramedic = vRP.NumPermission("Paramedico")
					local Police = vRP.NumPermission("Policia")

		    	    TriggerClientEvent("Notify",source,"amarelo","Há <b>"..parseInt(Paramedico).."</b> Paramedicos em serviço",10000)
...
RegisterCommand("p",function(source,Message,History)
	...
	if Passport and Message[1] and vRP.GetHealth(source) > 100 then
		if vRP.HasService(Passport,"Policia") then
			TriggerClientEvent("Notify",-1,"amarelo","<b>1º BPM de Hypex informa:</b> "..History:sub(2),30000)
...
RegisterCommand("h",function(source,Message,History)
	...
	if Passport and Message[1] and vRP.GetHealth(source) > 100 then
		if vRP.HasService(Passport,"Paramedico") then
			TriggerClientEvent("Notify",-1,"amarelo","<b>Centro Médico de Hypex informa:</b> "..History:sub(2),30000)

			TriggerEvent("Discord","Paramedico","**Passaporte:** "..Passport.."\n**Mensagem:** "..History:sub(2),9807270)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("m",function(source,Message,History)
	...
	if Passport and Message[1] and vRP.GetHealth(source) > 100 then
		if vRP.HasGroup(Passport,"Mecanico") then
			TriggerClientEvent("Notify",-1,"amarelo","<b>Mecânica informa:</b> "..History:sub(2),30000)

			TriggerEvent("Discord","Mecanico","**Passaporte:** "..Passport.."\n**Mensagem:** "..History:sub(2),9807270)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- M
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("Bennys",function(source,Message,History)
	local Passport = vRP.Passport(source)
	if Passport and Message[1] and vRP.GetHealth(source) > 100 then
		if vRP.HasGroup(Passport,"Bennys") then
			TriggerClientEvent("Notify",-1,"amarelo","<b>Bennys informa:</b> "..History:sub(2),30000)

			TriggerEvent("Discord","Bennys","**Passaporte:** "..Passport.."\n**Mensagem:** "..History:sub(2),9807270)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("h",function(source,Message,History)
	local Passport = vRP.Passport(source)
	if Passport and Message[1] and vRP.GetHealth(source) > 100 then
		if vRP.HasGroup(Passport,"Harmony") then
			TriggerClientEvent("Notify",-1,"amarelo","<b>Mecânica informa:</b> "..History:sub(2),30000)
...
local DuiTextures = {
	["MRPD"] = {
		...
		["Link"] = "https://Hypex-rp.com/Quadro.png",
...
-- SHOTSFIRED
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.shotsFired(Vehicle)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		if Vehicle then
