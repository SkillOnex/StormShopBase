...
-- REQUESTINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.requestInventory()
	local source = source
						...
						if Identity then
							...
							if Number == Passport and (Split[1] == "identity" or string.sub(v["item"],1,5) == "badge") then
								if Identity["premium"] > os.time() then
									...
									local numero = vRP.HasPermission(Passport,"Premium")
						
									v["idVality"] = MinimalTimers(Identity["premium"] - os.time())
									v["idPremium"] = Groups[numero]
...
-- DROPSERVER
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.DropServer(Coords,Item,Amount)
	local Number = 0
...
-- DROPS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.Drops(Item,Slot,Amount,x,y,z)
	local source = source
...
-- PICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.Pickup(Number,Amount,Slot)
	local source = source
	local Slot = tostring(Slot)
...
-- SENDITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.SendItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
...
-- TRASH
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.Trash(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
...
-- USEITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.UseItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
...
-- VERIFYWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.verifyWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- DROPWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.dropWeapons(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- REMOVETHROWING
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.removeThrowing(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- PREVENTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.preventWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- VERIFYOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.VerifyObjects(Entity,Service)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- STEALTRUNK
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.StealTrunk(Entity)
	local source = source
	local Plate = Entity[1]
...
-- STOREOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.StoreObjects(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- MAKEPRODUCTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.MakeProducts(Table)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- DISMANTLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.Dismantle(Entity)
	local source = source
...
-- REMOVETYRES
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.RemoveTyres(Entity)
	local source = source
	local Passport = vRP.Passport(source)
			...
			if vRP.TakeItem(Passport,"c4",1,true,Slot) then
				...
				repeat
					if os.time() >= parseInt(Active[Passport]) then
						...
						vRP.GenerateItem(Passport,"dollarsroll",math.random(290000,310000),true)
					end
...
-- REGISTERSTIMERS
-----------------------------------------------------------------------------------------------------------------------------------------
function Hypex.RegistersTimers(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
